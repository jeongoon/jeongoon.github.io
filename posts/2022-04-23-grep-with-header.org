...
#+begin_src sh
  sh> ps aux | head-width get-one-line --tail-with grep -i /fish/
      # or in fish-pandoc-any-to-markdown
  sh> cat some.org | head-width retrieve-metadata --tail-with pandoc -t markdown > some.md
#+end_src
